  ...
  return (
    <main className='bg-blue-gray-400 h-screen'>
        ...
        <div className='flex-1'>
            <div>
                <div className='flex gap-10 max-lg:absolute max-lg:top-0'>
                    ...
                    <h2><p className='font-serif text-5xl absolute mt-5'>{influencer.username}</p></h2>
                </div>
                <div className='flex ml-20 mt-8 gap-32 max-xl:gap-20 max-lg:ml-0 
                    ...
                    <div className='mt-10'>
                        {influencer.platforms.map((platform) => {
                            ...
                            return (
                                ...
                                <img src={youtube} width={40} className='mb-5' alt='YouTube Icon' />
                                <p className='mt-1 ml-3 font-serif text-xl'>{platform.followerCount} Followers</p>
                                </div>
                            );
                            }
                            ...
                            return (
                                <div className='flex' key={platform.id}>
                                <img src={instagram} width={40} className='mb-5' alt='Instagram Icon' />
                                <p className='mt-1 ml-3 font-serif text-xl'>{platform.followerCount} Followers</p>
                                </div>
                            );
                            }
                ...
                max-lg:justify-center max-md:flex max-md:justify-center max-lg:mb-3'>
                    <button className='ml-36 mt-2 bg-blue-500 px-5 py-2 rounded-full font-serif 
                    text-white max-lg:mb-3 max-lg:ml-0 max-md:ml-0'>Contact</button>
                </div>
            </div>
        </div><hr className='border-2 hidden max-lg:block'/>
      ...
      <div className='px-5 grid grid-cols-3 gap-5 py-5 max-md:grid-cols-2 max-sm:grid-cols-1 bg-blue-gray-400'>
          ...
          {commentData.filter(comment => comment.receiverId === receiverId).map((comment) => {
            const company = companyData.find(company => company._id === comment.senderId);
            return (
            <div className='bg-white w-full rounded-2xl shadow-xl flex'>
              <div className='p-4' key={comment._id}>
                  <div className='flex'>
                      ...
                      {company && <p className='ml-2 mt-2'>{company.contactName}</p>}
                  </div>
      ...
      <div className='flex'>
        <div className='fixed bottom-1 right-16 max-sm:right-16 border-2 border-gray-300 h-16 rounded-md bg-white p-2'>
          ...
          <div className='flex gap-2 absolute right-2 mt-1 cursor-pointer'>
              ...
              <div>
              <img src={upload} width={20} onClick={uploadComment} alt="upload" />
              </div>
          </div>
        </div>
      </div>
